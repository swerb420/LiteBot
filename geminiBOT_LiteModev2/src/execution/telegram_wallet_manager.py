...
class WalletManager:
    ...
    def _setup_handlers(self):
        self.bot.application.add_handler(CommandHandler("wallets", self.list_wallets_command))
        self.bot.application.add_handler(CommandHandler("wallet", self.wallet_details_command))
        self.bot.application.add_handler(CommandHandler("analyze", self.analyze_wallet_command))
        self.bot.application.add_handler(CommandHandler("patterns", self.patterns_command))
        self.bot.application.add_handler(CommandHandler("leaderboard", self.leaderboard_command))

        add_wallet_conv = ConversationHandler(
            entry_points=[CommandHandler("add_wallet", self.add_wallet_start)],
            states={
                WALLET_ADDRESS: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.add_wallet_address)],
                WALLET_LABEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.add_wallet_label)],
                WALLET_CATEGORY: [CallbackQueryHandler(self.add_wallet_category)],
                WALLET_TAGS: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.add_wallet_tags)],
        ...
        edit_wallet_conv = ConversationHandler(
            ...
            states={
                ...
                EDIT_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.edit_wallet_value)]
            },
            fallbacks=[CommandHandler("cancel", self.cancel)]
        )
        self.bot.application.add_handler(edit_wallet_conv)

        self.bot.application.add_handler(CommandHandler("remove_wallet", self.remove_wallet_command))
    ...
    async def add_wallet_address(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        address = update.message.text.strip()
        if not re.match(r'^0x[a-fA-F0-9]{40}$', address):
            await update.message.reply_text("Invalid address. Try again")
            return WALLET_ADDRESS
        existing = await db.fetchrow("SELECT * FROM tracked_wallets WHERE wallet_address=$1", address)
        if existing:
    ...
    async def add_wallet_label(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [[
            ...
            InlineKeyboardButton("Institution", callback_data="category_institution"),
    ...
    async def add_wallet_category(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def add_wallet_tags(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        tags = [] if text.lower() == 'skip' else [t.strip() for t in text.split(',') if t.strip()]
    ...
    async def confirm_add_wallet(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
    ...
    async def wallet_details_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
    ...
    async def analyze_wallet_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
            await update.message.reply_text("Usage: /analyze <wallet>")
            return
        search_term = context.args[0]
        wallet = await db.fetchrow("SELECT * FROM tracked_wallets WHERE wallet_address ILIKE $1||'%' OR label ILIKE $1||'%' LIMIT 1", search_term)
        if not wallet:
            await update.message.reply_text("Wallet not found")
            return
        await update.message.reply_text("Running AI analysis...")
        analysis = await self.analyzer.comprehensive_ai_analysis(wallet['wallet_address'])
        if analysis.get('error'):
            await update.message.reply_text(analysis['error'])
            return
        await update.message.reply_text(f"Primary strategy: {analysis['behavior']['primary_strategy']}")
    ...
    async def edit_wallet_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
        ...
        await update.message.reply_text("Select field to edit", reply_markup=InlineKeyboardMarkup(keyboard))
        return EDIT_FIELD

    async def edit_wallet_field(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
    ...
    async def edit_wallet_value(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
